...
export function mercureOpen(eventSource) {
  return { type: 'COMPANY_LIST_MERCURE_OPEN', eventSource };
}

export function mercureMessage(retrieved) {
  return dispatch => {
    if (1 === Object.keys(retrieved).length) {
      dispatch({ type: 'COMPANY_LIST_MERCURE_DELETED', retrieved });
      return;
    }

    dispatch({ type: 'COMPANY_LIST_MERCURE_MESSAGE', retrieved });
  };
}
